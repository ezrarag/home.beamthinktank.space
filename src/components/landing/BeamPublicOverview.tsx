...
export function BeamPublicOverview() {
  ...
  useEffect(() => {
    ...
    async function loadNodes() {
      try {
        ...
        if (!response.ok) throw new Error(`Failed to load public nodes: ${response.status}`);
        const payload = (await response.json()) as { nodes?: BeamNode[] };
        if (!cancelled) setPublicNodes(Array.isArray(payload.nodes) ? payload.nodes : []);
      } catch {
        if (!cancelled) setPublicNodes([]);
      }
    }
