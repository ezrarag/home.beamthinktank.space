...
export default function WorkstationPage() {
  ...
  useEffect(() => {
    setRecordingUrl(null); setTranscriptUrl(null);
    if (!expanded || !canViewExpanded) return;
    if (expanded.recordingStoragePath) void getDownloadURL(ref(getFirebaseStorage(), expanded.recordingStoragePath)).then(setRecordingUrl).catch(() => undefined);
    if (expanded.transcriptStoragePath) void getDownloadURL(ref(getFirebaseStorage(), expanded.transcriptStoragePath)).then(setTranscriptUrl).catch(() => undefined);
  }, [canViewExpanded, expanded]);
  useEffect(() => {
    ...
    <main className="min-h-screen bg-[#090a09] text-[#f0ead6]">
      ...
      <div className="mx-auto max-w-7xl px-5 py-10 sm:px-8">
        ...
        <div className="grid gap-8 py-8 lg:grid-cols-[minmax(0,1fr)_22rem]"><div className="space-y-7">
          {(recordingUrl || expanded.recordingDriveFileId || transcriptUrl || expanded.transcriptDriveUri) ? <section className="beam-card rounded-[1.75rem] p-6 sm:p-8"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="beam-eyebrow">Meeting record</p><h2 className="beam-display mt-2 text-3xl">Watch, read, verify.</h2></div>{expanded.sourceRevision ? <span className="rounded-full border border-white/10 px-3 py-1 font-mono text-[10px] text-white/40">rev {expanded.sourceRevision}</span> : null}</div>{recordingUrl ? <video src={recordingUrl} controls preload="metadata" className="mt-6 aspect-video w-full rounded-2xl bg-black"/> : expanded.recordingDriveFileId ? <iframe title={`${expanded.title} recording`} src={`https://drive.google.com/file/d/${expanded.recordingDriveFileId}/preview`} allow="autoplay" className="mt-6 aspect-video w-full rounded-2xl border-0 bg-black"/> : null}<div className="mt-5 flex flex-wrap gap-3 text-xs">{transcriptUrl ? <a href={transcriptUrl} target="_blank" rel="noreferrer" className="rounded-full border border-white/15 px-4 py-2 text-[var(--beam-gold)]">Open transcript snapshot ↗</a> : null}{expanded.transcriptDriveUri ? <a href={expanded.transcriptDriveUri} target="_blank" rel="noreferrer" className="rounded-full border border-white/15 px-4 py-2 text-white/65">Open source notes in Drive ↗</a> : null}{expanded.recordingDriveUri ? <a href={expanded.recordingDriveUri} target="_blank" rel="noreferrer" className="rounded-full border border-white/15 px-4 py-2 text-white/65">Open source recording ↗</a> : null}</div><p className="mt-4 text-[10px] leading-5 text-white/30">The revision label identifies the exact transcript version used for this meeting record. Drive remains the source; Firebase snapshots keep published materials stable.</p></section> : null}
