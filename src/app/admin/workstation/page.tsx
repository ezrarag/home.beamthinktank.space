...
const EMPTY_FORM: FormState = {
  ...
  takeaways: [], invitees: [], restrictToInvitees: false, order: 1, meetSpaceUri: null, meetSpaceName: null,
  ...
  sourceFolderUri: null, recordingDriveFileId: null, transcriptDriveFileId: null, sourceRevision: null, sourceImportedAt: null,
};
...
const PUBLIC_WORKSTATION_URL = "https://beamthinktank.space/workstation";
...
export default function AdminWorkstationPage() {
  ...
  const [driveRecordingUrl, setDriveRecordingUrl] = useState("");
  const [driveTranscriptUrl, setDriveTranscriptUrl] = useState("");
  const [copyDriveRecording, setCopyDriveRecording] = useState(false);
  const [drivePreview, setDrivePreview] = useState<{ summary: string; decisions: string; nextSteps: string; revision: string; recordingName: string; transcriptName: string } | null>(null);
  ...
  function editMeeting(meeting: BeamMeeting) {
    ...
    setEditingId(id); setForm(values); setInviteesText(values.invitees.join("\n")); setTakeawaysMarkdown(""); setParsedTakeaways([]); setDriveFolderUrl(values.sourceFolderUri || ""); setDriveRecordingUrl(values.recordingDriveUri || ""); setDriveTranscriptUrl(values.transcriptDriveUri || ""); setDrivePreview(null); setMessage(null); setError(null);
  }

  function newMeeting() {
    setEditingId(null); setForm({ ...EMPTY_FORM, order: meetings.length + 1 }); setInviteesText(""); setSuggestions([]); setTakeawaysMarkdown(""); setParsedTakeaways([]); setDriveFolderUrl(""); setDriveRecordingUrl(""); setDriveTranscriptUrl(""); setDrivePreview(null); setMessage(null); setError(null);
  }
  ...
  async function copyInviteLink(meetingId: string) {
    try {
      await navigator.clipboard.writeText(`${PUBLIC_WORKSTATION_URL}?meeting=${encodeURIComponent(meetingId)}`);
      setMessage("Invite link copied.");
      setError(null);
    } catch {
      setError("The invite link could not be copied. Your browser may be blocking clipboard access.");
    }
  }
  ...
  async function ingestDrive(action: "inspect" | "import") {
    if (!editingId) { setError("Save the meeting before connecting Drive files."); return; }
    setBusyAction(`drive-${action}`); setError(null); setMessage(null);
    try {
      const [firebaseToken, googleToken] = await Promise.all([getFirebaseAuth().currentUser!.getIdToken(true), googleAccessToken()]);
      const response = await fetch(`/api/admin/workstation/meet/${editingId}/drive`, {
        method: "POST",
        headers: { Authorization: `Bearer ${firebaseToken}`, "x-google-access-token": googleToken, "Content-Type": "application/json" },
        body: JSON.stringify({ action, folderUrl: driveFolderUrl, recordingUrl: driveRecordingUrl, transcriptUrl: driveTranscriptUrl, copyRecording: copyDriveRecording }),
      });
      const payload = await response.json() as { error?: string; preview?: typeof drivePreview; meeting?: Partial<FormState>; copiedRecording?: boolean };
      if (!response.ok) throw new Error(payload.error || "Drive ingestion failed.");
      if (payload.preview) setDrivePreview(payload.preview);
      if (payload.meeting) setForm((current) => ({ ...current, ...payload.meeting }));
      setMessage(action === "inspect" ? "Drive sources inspected. Review the extracted meeting record below before importing." : payload.copiedRecording ? "Drive sources versioned and the recording copied to Firebase Storage." : "Drive sources versioned. The recording remains available through its Drive preview.");
    } catch (nextError) { setError(nextError instanceof Error ? nextError.message : "Drive ingestion failed."); }
    finally { setBusyAction(null); }
  }
  ...
  return (
    <main className="min-h-screen bg-[#0e0e0e] px-5 py-10 text-white sm:px-8">
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[20rem_minmax(0,1fr)]">
        <aside><div className="flex items-center justify-between"><div><p className="beam-eyebrow">BEAM Admin</p><h1 className="beam-display mt-2 text-3xl">Meetings</h1></div><button onClick={newMeeting} className="rounded-full bg-[var(--beam-gold)] px-3 py-2 text-xs font-semibold text-black">+ New</button></div><div className="mt-6 space-y-2">{meetings.map((meeting) => <div key={meeting.id} className={`rounded-xl border p-4 ${editingId === meeting.id ? "border-[var(--beam-gold)] bg-white/5" : "border-white/10"}`}><button type="button" onClick={() => editMeeting(meeting)} className="w-full text-left"><span className="text-[10px] uppercase tracking-wider text-[var(--beam-gold)]">{meeting.status}{meeting.restrictToInvitees ? " · restricted" : ""}</span><p className="mt-1 text-sm">{meeting.title}</p><p className="mt-1 text-[10px] text-white/35">{new Date(meeting.dateISO).toLocaleString()}</p></button><button type="button" onClick={() => void copyInviteLink(meeting.id)} className="mt-3 text-[10px] text-[var(--beam-gold)]">Copy invite link</button></div>)}</div><Link href="/admin" className="mt-6 inline-flex text-xs text-white/45 hover:text-white">← Admin portal</Link></aside>
        <form onSubmit={saveMeeting} className="beam-card rounded-[1.75rem] p-6 sm:p-8">
          ...
          <div className="mt-7 grid gap-5 sm:grid-cols-2">
            ...
            <label className="sm:col-span-2 flex items-start gap-3 rounded-xl border border-white/10 bg-black/20 p-4 text-xs text-white/65"><input type="checkbox" checked={form.restrictToInvitees} onChange={(e) => setForm({ ...form, restrictToInvitees: e.target.checked })} className="mt-0.5"/><span><strong className="block text-white">Restrict to invitees only</strong><span className="mt-1 block text-white/40">Only listed email addresses and administrators can open meeting details and files.</span></span></label>
          </div>
          ...
          <section className="mt-8 border-t border-white/10 pt-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-medium">Google Drive evidence</h3><p className="mt-1 max-w-2xl text-xs leading-5 text-white/40">Connect the original folder, recording, and Gemini notes. Inspect first; import records a source revision without replacing your agenda or takeaways.</p></div>{form.sourceRevision ? <span className="rounded-full border border-emerald-400/25 bg-emerald-400/10 px-3 py-1 text-[10px] text-emerald-200">Revision {form.sourceRevision}</span> : null}</div><div className="mt-4 grid gap-3"><input value={driveFolderUrl} onChange={(e)=>setDriveFolderUrl(e.target.value)} placeholder="Google Drive folder URL" className="rounded-xl border border-white/10 bg-black/30 p-3 text-xs"/><input value={driveRecordingUrl} onChange={(e)=>setDriveRecordingUrl(e.target.value)} placeholder="Google Drive recording URL" className="rounded-xl border border-white/10 bg-black/30 p-3 text-xs"/><input value={driveTranscriptUrl} onChange={(e)=>setDriveTranscriptUrl(e.target.value)} placeholder="Google Docs transcript / Gemini notes URL" className="rounded-xl border border-white/10 bg-black/30 p-3 text-xs"/></div><label className="mt-4 flex items-start gap-3 rounded-xl border border-white/10 bg-black/20 p-4 text-xs text-white/55"><input type="checkbox" checked={copyDriveRecording} onChange={(e)=>setCopyDriveRecording(e.target.checked)} className="mt-0.5"/><span><strong className="block text-white">Copy video into Firebase Storage</strong><span className="mt-1 block text-white/35">Optional. Leave off to use the Drive player and avoid duplicating a large recording; turn on when viewers should not depend on Drive permissions.</span></span></label><div className="mt-4 flex flex-wrap gap-2"><button type="button" disabled={!editingId || busyAction !== null || !driveTranscriptUrl} onClick={()=>void ingestDrive("inspect")} className="rounded-full border border-white/15 px-4 py-2 text-xs">{busyAction === "drive-inspect" ? "Inspecting..." : "Inspect sources"}</button><button type="button" disabled={!editingId || busyAction !== null || !driveTranscriptUrl} onClick={()=>void ingestDrive("import")} className="rounded-full bg-[var(--beam-gold)] px-4 py-2 text-xs font-semibold text-black">{busyAction === "drive-import" ? "Importing..." : "Import reviewed sources"}</button></div>{drivePreview ? <div className="mt-5 grid gap-3 lg:grid-cols-3"><article className="rounded-xl border border-white/10 bg-black/20 p-4"><p className="text-[10px] uppercase tracking-wider text-[var(--beam-gold)]">Summary</p><p className="mt-2 whitespace-pre-wrap text-xs leading-5 text-white/55">{drivePreview.summary || "None found"}</p></article><article className="rounded-xl border border-white/10 bg-black/20 p-4"><p className="text-[10px] uppercase tracking-wider text-[var(--beam-gold)]">Decisions</p><p className="mt-2 whitespace-pre-wrap text-xs leading-5 text-white/55">{drivePreview.decisions || "None found"}</p></article><article className="rounded-xl border border-white/10 bg-black/20 p-4"><p className="text-[10px] uppercase tracking-wider text-[var(--beam-gold)]">Next steps</p><p className="mt-2 whitespace-pre-wrap text-xs leading-5 text-white/55">{drivePreview.nextSteps || "None found"}</p></article></div> : null}</section>
          ...
          <div className="mt-8 flex flex-wrap gap-3"><button type="submit" className="rounded-full bg-[var(--beam-gold)] px-6 py-3 text-xs font-semibold uppercase tracking-wider text-black">Save meeting</button>{editingId ? <button type="button" onClick={() => void copyInviteLink(editingId)} className="rounded-full border border-white/15 px-6 py-3 text-xs uppercase tracking-wider text-white/70">Copy invite link</button> : null}</div>
        </form>
      </div>
    </main>
  );
