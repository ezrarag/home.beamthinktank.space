    ...
    <main className="min-h-screen w-full bg-[#0e0e0e] text-white">
      <div className="mx-auto w-full max-w-5xl px-6 py-10 sm:px-8">
        <header className="border-b border-white/10 pb-6">
          ...
          <h1 className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">Law Processes</h1>
          <p className="mt-3 max-w-3xl text-sm text-white/70 sm:text-base">
            Track legal filings, entity registrations, trademarks, and per-state compliance for BEAM.
          </p>
        ...
        <section className="mt-8 space-y-6">
          {!isSignedIn ? (
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-8 text-center text-white/65">
              <p className="text-sm">Sign in with an admin account to view law processes.</p>
              <button
                type="button"
                onClick={handleGoogleSignIn}
              ...
              {loading ? (
                ...
                !isCreating && (
                  <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 text-sm text-white/65">
                    No law processes found. Create a new process or seed the collection with <code>scripts/seedLawProcesses.ts</code>.
                  </div>
                )
              )}
            </>
